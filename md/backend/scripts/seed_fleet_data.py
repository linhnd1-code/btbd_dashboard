...
from services.fleet_parser import (  # noqa: E402
    parse_maintenance_records,
    parse_maintenance_status,
    parse_vehicles,
)

DATA_FILE = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "fleet_sheet_raw.md")
...
def seed() -> None:
    ...
    print(f"Đã import {len(records)} nhật ký BTBD, {len(vehicles)} xe, {len(statuses)} trạng thái bảo dưỡng.")
